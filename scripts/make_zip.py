...
def  main():
  ...
  installer = "\installer\Sputter Installer.exe"
   
  if demo:
    installer = "\installer\Sputter Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Sputter manual.pdf" 
  ]
  ...
  ZIPNAME = "Sputter-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Sputter-v" + FULLVERSIONSTR + "-win-demo.zip"
